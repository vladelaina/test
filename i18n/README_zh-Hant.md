...
#### 📦 2.1 安裝 [MinGW Build](https://github.com/niXman/mingw-builds-binaries/releases/latest)

1. **將 MinGW 解壓縮到特定位置**
   範例：將 `x86_64-<version>-release-win32-seh-ucrt-rtv<num>-rev<num>.7z` 解壓縮至：
...
3. **驗證安裝**
   ...
   開啟命令提示字元（Win + R → 輸入 `cmd` → Enter），輸入：
