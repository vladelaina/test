...
#### <img src="../Images/windows.svg"  height="25" />Windows
  ...
  #### 📦 2.1 [MinGW Build](https://github.com/niXman/mingw-builds-binaries/releases/latest)のインストール

  1. **MinGWを特定の場所に解凍**
     例：x86_64-<version>-release-win32-seh-ucrt-rtv<num>-rev<num>.7z を以下に解凍：
  ...
  2. **システム環境変数PATHの設定**
     ...
     * 開く：コントロールパネル → システム → システムの詳細設定 → 環境変数
     * **システム変数**の Path を探し、「編集」をクリック
     * 以下のパスを追加：
  ...
  3. **インストールの確認**
     ...
     コマンドプロンプトを開き（Win + R → 「cmd」と入力 → Enter）、以下を入力：
